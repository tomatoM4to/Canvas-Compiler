export default function chatGPT(prompt: string) {
    ...
    commands.push("넌 지금 내가 주는 데이터를 가지고 코드를 작성하는 역할을 맡았어 만약 명령하는 정보의 값이 undefined일시 디폴트 옵션으로 코드를 작성해");

    if (konvaData.attrs && konvaData.attrs.id) {
        commands.push(konvaData.attrs.id);
    }
                ...
                if (node.attrs.name === 'rect') {
                    switch (node.className) {
                        case 'Rect':
                            ...
                            break;
                        case 'Text':
                            currentCommand = `${idx++} 해당 컴포넌트는 "${node.attrs.text || "undefined"}" 라는 Text 컴포넌트야
